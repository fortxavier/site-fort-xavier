    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          tipo_imovel: data.tipo_imovel || [], // Novo campo
          endereco: data.endereco || '',
          texto_localizacao: data.texto_localizacao || '',
          valor_condominio: data.valor_condominio?.toString() || '',
          caracteristicas: data.caracteristicas || [],
          banheiros: data.banheiros || [], // Volta para array
          quartos: data.quartos || [], // Volta para array
          vagas: data.vagas || [], // Volta para array
      ...
      setMessage('Erro ao carregar dados da propriedade.');
  ...
  const handleDeleteCardImage = async () => {
    try {
      ...
      if (success) {
        ...
        setMessage('Imagem de card deletada com sucesso!');
        setMessageType('success');
      } else {
        setMessage('Erro ao deletar imagem de card.');
        setMessageType('error');
      }
    } catch (error) {
  ...
  const handleDeleteCoverImage = async () => {
    try {
      ...
      if (success) {
        ...
        setMessage('Imagem de capa deletada com sucesso!');
        setMessageType('success');
      } else {
        setMessage('Erro ao deletar imagem de capa.');
        setMessageType('error');
      }
    } catch (error) {
...
const handleRemoveGalleryImage = async (imageUrl: string, galleryName: string): Promise<boolean> => {
  try {
    ...
    if (success) {
      ...
      setMessage(`Imagem removida da galeria "${galleryName}" com sucesso!`);
      setMessageType('success');
      return true;
    } else {
      setMessage(`Erro ao remover imagem da galeria "${galleryName}".`);
      setMessageType('error');
      return false;
    }
  } catch (error) {
...
const handleDeleteGallery = async (galleryName: string): Promise<boolean> => {
  try {
    ...
    if (success) {
      ...
      setMessage(`Galeria "${galleryName}" deletada com sucesso!`);
      setMessageType('success');
      return true;
    } else {
      setMessage(`Erro ao deletar galeria "${galleryName}".`);
      setMessageType('error');
      return false;
    }
  } catch (error) {
          ...
          try {
            ...
            if (files.length > 0) {
              ...
              if (galleryResult.success) {
                ...
                // Recarregar imagens da galeria para atualizar com as URLs do servidor
                await loadExistingImages();
              } else {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '23505' && error.details?.includes('slug')) {
          ...
          setMessage(`Erro ao atualizar propriedade: ${error.message}`);
        }
        setMessageType('error');
        return;
      }
      ...
      if (hasNewImages) {
        setMessage('Propriedade atualizada! Fazendo upload das novas imagens...');
        setMessageType('info');
        ...
        if (failedUploads.length > 0) {
          setMessage(`Propriedade atualizada, mas houve problemas no upload de algumas imagens. Verifique e tente novamente.`);
          setMessageType('warning');
        } else {
          setMessage('Propriedade e imagens atualizadas com sucesso!');
          setMessageType('success');
        }
      } else {
        setMessage('Propriedade atualizada com sucesso!');
        setMessageType('success');
      }
      ...
      setMessage('Erro inesperado ao atualizar propriedade.');
      setMessageType('error');
    }
  ...
  return (
    <div className="fade-in property-edit-container">
      ...
      <div className="page-header">
        <h1 className="page-title">Editar Propriedade</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-card">
          ...
          <MultiGalleryManager
            ...
            onRemoveImage={handleRemoveGalleryImage} // Nova prop
            maxGalleries={8}
